  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      if (postsWithAuthors.length > 0) {
        setFeaturedPost(postsWithAuthors[0]);
        setPosts(postsWithAuthors.slice(1));
      }
    } catch (error) {
              ...
              {post.author && (
                <>
                  ...
                  <span className="text-muted-foreground">
                    {post.author.display_name || "AfrikSpark"}
                  </span>
                </>
              )}
            </div>
                            ...
                            {featuredPost.author && (
                              <div className="flex items-center gap-2">
                                <Avatar className="h-8 w-8">
                                  ...
                                  <AvatarFallback>
                                    {featuredPost.author.display_name?.[0]}
                                  </AvatarFallback>
                                </Avatar>
                ...
                {getPopularPosts().map((post) => (
                  <AnimateOnScroll key={post.id}>
                    <div className="relative">
                      <TrendingUp className="absolute -top-2 -right-2 h-8 w-8 text-primary" />
                      <BlogCard post={post} />
                    </div>
                  </AnimateOnScroll>
                ))}
